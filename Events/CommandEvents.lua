...
function CommandEvents:run(connection) -- wir fuehren das empfangene event aus
	CpManager[self.functionName](self.parameter)
...
function CommandEvents.sendEvent(functionName,parameter)
    if g_server ~= nil then
        ...
        g_server:broadcastEvent(CommandEvents:new(functionName,parameter))
		courseplay.debugFormat(5,"CommandEvents: broadcast event ")
    end
end